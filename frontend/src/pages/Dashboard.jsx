...
export default function Dashboard() {
  ...
  // Use username if available, fall back to name, then a generic greeting
  ...
  useEffect(() => {
    ...
    supabase
      ...
      .then(({ data }) => {
        if (data?.length) {
          // Most recent reading for metric cards
          setLatest(data[0]);
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Plant selector shown only when plants exist */}
      {plants.length > 0 && (
        <select
          value={selectedId}
      ...
      {/* Message when user has no plants yet */}
      {hasNoPlants && (
      ...
      {/* Message when plant exists but has no linked device */}
      ...
      {/* Message when device is linked but no readings received yet */}
